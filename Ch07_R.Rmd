...
(d) Project the $2^{5-2}$ design to only considering factors $A$, $B$ and $D$. The estimation of the effects are:
```{r r811-4, eval=TRUE, echo=TRUE, fig.align='center', fig.width=5}
# Compute the model matrix of all effect terms without intercept
mmat_proj <- model.matrix( ~ A*B*D - 1, data = frfData)
# Calculate the effect sizes using the +/- signs of the model matrix
eff_proj <- numeric(ncol(mmat_proj))
for (i in 1:ncol(mmat_proj)) {
  eff_proj[i] <- 2*mean(frfData$y*mmat_proj[,i])
}
names(eff_proj) <- colnames(mmat_proj)
```

```{r, eval=TRUE, echo=FALSE, fig.align='center', fig.width=5}
print(data.frame(
  Factor = names(eff_proj),
  "Est.Effect" = as.vector(eff_proj)
))
```
...
```{r r811-5, eval=TRUE, echo=TRUE, fig.align='center', fig.width=5}
fitProj <- aov(y ~ A+B+D+B:D+A:B:D, data = frfData)
summary(fitProj)
```


```{r r811-5, eval=TRUE, echo=TRUE, fig.align='center', fig.width=5}
fitProj <- aov(y ~ A+B+C+D+E+A:B+A:D, data = frfData)
summary(fitProj)
```


```{r r811-5, eval=TRUE, echo=TRUE, fig.align='center', fig.width=5}
fitProj <- aov(y ~ A:B+A:D, data = frfData)
summary(fitProj)
```
