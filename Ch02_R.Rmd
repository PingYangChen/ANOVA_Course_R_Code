...
```{r, eval=TRUE, echo=TRUE, fig.align='center', fig.width=6}
...
The boxplot shows that: `ModifiedMortar` has lower median tension bond strength with larger variation, and, `UnmodifiedMortar` has higher and more consistent tension bond strength.
...
```
```{r vartest, eval=TRUE, echo=FALSE, fig.align='center'}
vartest <- var.test(df1$ModifiedMortar, df1$UnmodifiedMortar,
                    alternative = "two.sided")
```
The F-test shows a large p-value, `r round(vartest$p.value, 4)`, and hence, we do not reject $H_{V0}$. That is, there is no evidence to conclude the variances of the two populations, strength of modified and original Mortar, are different.
...
For paired t-test, we check the Normality of the $d_j = y_{1j} - y_{2j}$. According to the QQ plots, Normality check is passed because most of the observed difference between hardness of tips on the QQ plots are closed to the straight line.
```{r, eval=TRUE, echo=TRUE, fig.align='center'}
tip_diff <- df3$Tip1 - df3$Tip2
qqnorm(tip_diff, main = "QQ Plot of Tips Hardness Difference")
qqline(tip_diff, col = "red")
```
